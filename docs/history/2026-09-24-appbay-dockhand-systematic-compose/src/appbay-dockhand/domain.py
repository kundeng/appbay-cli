...
SITES = [
    ...
    ("compile-orchestrator", "appbay-cli", "packages/core/src/compiler/compile.ts", 200, "compile", "pipeline"),
...
BRIDGE = [
    # ⚠️ ONE DOMAIN LABEL PER BRIDGE TYPE. ABOUT also pointed at Product, which made
    # the ABOUT table multi-pair and LadybugDB 0.20.4 refuses an unnamed bulk COPY into
    # one. The product is reachable from any CodeSite through SITE_IN, so this costs a
    # hop rather than a fact.
    ("ABOUT", "F1", "CodeSite", "compile-orchestrator"),
    ("ABOUT", "F2", "CodeSite", "dh-deploy"),
