...
import { VERSION, compareSemver, containerCompose, discoverApps, isSystemApp, composeProject } from "@appbay/core";
...
async function pullSystemImages(): Promise<number> {
  ...
  for (const app of rendered) {
    ...
    const pull = containerCompose(["-p", composeProject(app.name), "pull", ...pullable], app.render, undefined, appbayHome);
    if (pull.exitCode === 0) {
