...
export function buildTraefikConfig(
  ...
  const serviceUrl = `http://${appName}_${serviceName}:${port}`;
 ...
 */
export function traefikAuxPath(appName: string): string {
  return `etc/apps/traefik/config/dynamic/${appName}.yml`;
}
 ...
 */
export function caddyAuxPath(appName: string): string {
  return `etc/apps/caddy/config/dynamic/${appName}.caddy`;
}
...
export function buildCaddySnippet(
  ...
  const upstream = `${appName}_${serviceName}:${String(props.port)}`;
  return [
...
export const ingressTraitDefinition: TraitDefinition<"ingress"> = {
  ...
  transform(input: TraitTransformInput): TraitTransformOutput {
    ...
    const { path, content } =
      provider === "caddy"
        ? {
            path: caddyAuxPath(appName),
            content: buildCaddySnippet(appName, serviceName, props),
          }
        : {
            path: traefikAuxPath(appName),
            content: yamlStringify(buildTraefikConfig(appName, serviceName, props), {
              sortMapEntries: true,
            }),
          };
