...
export function buildCaddySnippet(
  ...
  //
  // ⚠️ `upstreamAlias` overrides that derivation for the one caller that is NOT compiled
  // through the pipeline — the control plane declares its own alias on `appbay_shared`
  // (see services/control-plane-edge.ts). An explicit parameter rather than a string
  // rewrite at the call site: a rewrite that stops matching this shape would silently emit
  // a site block pointing at a host that does not exist.
  const upstream = `${upstreamAlias ?? sharedNetworkAlias(namespace, appName, serviceName)}:${String(props.port)}`;
  return [
